...
def load_apps(data_dir: Path) -> dict[str, AppEntry]:
    ...
    for key, value in data.get("apps", {}).items():
        result[key] = AppEntry(
            key=key,
            display_name=value.get("display_name") or value.get("name", key),
            path=value.get("path", ""),
            args=value.get("args") or [],
            process_names=value.get("process_names") or [],
...
def open_allowed_app(data_dir: Path, app_key: str) -> tuple[bool, str]:
    ...
    if not app:
        return False, "Приложение не найдено."

    if not app.path:
        return False, f"Не найден путь для {app.display_name}."
    ...
    try:
        ...
        return True, f"{app.display_name} открыт."
    except FileNotFoundError:
        return False, f"Не найден путь для {app.display_name}."
    except Exception as exc:
        return False, f"Ошибка запуска {app.display_name}: {str(exc)}"
