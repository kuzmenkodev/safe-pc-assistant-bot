...
def is_authorized(config: AppConfig, user_id: int | None, chat_id: int | None) -> bool:
    ...
    user_ok = (
        not config.authorized_user_ids
        or user_id in config.authorized_user_ids
    )
    chat_ok = (
        not config.authorized_chat_ids
        or (chat_id is not None and chat_id in config.authorized_chat_ids)
    )
